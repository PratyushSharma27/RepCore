...
function AdminPage() {
  // Tabs management
  const [activeTab, setActiveTab] = useState<"dashboard" | "orders" | "products" | "categories" | "customers" | "coupons">("dashboard");
  ...
  const maxDiscountLabel = coupons.reduce((max, c) => {
  ...
  const openNew = () => {
    ...
    setEditing({
      slug: "", name: "", tagline: "", price: 0, category: categories[0]?.name || "", image: "",
  ...
  const openNewCoupon = () => {
    ...
    setEditingCoupon({
      code: "", type: "percentage", value: 10, minOrder: 0, active: true, description: ""
    });
    setCouponOpen(true);
  };
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    e.preventDefault();
    const slug = newCatId.trim().toLowerCase().replace(/[^a-z0-9]/g, "-");
  ...
  if (!user || !isAdmin) {
    return (
      <SiteLayout>
        <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[500px] h-[500px] pointer-events-none opacity-20 blur-3xl -z-10"
          ...
          {user ? (
            ...
            <div className="group overflow-hidden rounded-3xl border border-border/60 bg-card/60 backdrop-blur-md p-8 shadow-xl text-center relative">
              ...
              <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
                You are currently signed in as <span className="font-semibold text-foreground">{user.email}</span>. Only <strong className="text-primary font-bold">pratyush@tenimal.com</strong> can access the administrator control dashboard.
              </p>
              <div className="mt-8 flex flex-col gap-3">
                <Button variant="hero" onClick={() => navigate({ to: "/" })} className="btn-lift w-full">
                  Back to Storefront
                </Button>
                <Button variant="outline" onClick={logout} className="btn-lift w-full border-destructive/20 hover:bg-destructive/5 text-destructive">
            ...
            <div className="group overflow-hidden rounded-3xl border border-border/60 bg-card/60 backdrop-blur-md p-8 shadow-xl relative">
              ...
              <form onSubmit={handleLogin} className="mt-8 space-y-4">
                ...
                <Button type="submit" disabled={loginLoading} variant="hero" className="w-full mt-6 btn-lift glow-pulse flex items-center justify-center gap-2">
                  {loginLoading ? (
                    <>
  ...
  return (
    <SiteLayout>
      <section className="relative overflow-hidden border-b border-border/60 particle-field">
        <div className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-4 px-4 py-16 sm:px-6">
          <div>
            ...
            <p className={`mt-3 text-muted-foreground anim-reveal-up anim-delay-2 ${entered ? "visible" : ""}`}>
          ...
          <div className={`flex gap-3 anim-reveal-up anim-delay-3 ${entered ? "visible" : ""}`}>
            ...
            {activeTab === "coupons" && (
              <Button variant="hero" size="lg" onClick={openNewCoupon} className="btn-lift glow-pulse">
      ...
      {activeTab === "dashboard" && (
        <>
          <section className="mx-auto grid max-w-7xl gap-4 px-4 py-8 sm:px-6 md:grid-cols-4 perspective-container">
            <StatCard icon={<TrendingUp />} label="Total Sales" value={`₹${totalSales.toLocaleString("en-IN")}`} visible={tabAnimate} index={0} />
            <StatCard icon={<ShoppingBag />} label="Orders Count" value={String(orders.length)} visible={tabAnimate} index={1} />
            <StatCard icon={<Users />} label="Registered Customers" value={String(customers.length)} visible={tabAnimate} index={2} />
            <StatCard icon={<DollarSign />} label="Avg Order Value" value={`₹${avgOrderValue.toLocaleString("en-IN")}`} visible={tabAnimate} index={3} />
          </section>
          ...
          <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 grid gap-6 md:grid-cols-3">
                  ...
                  <tbody>
                    {orders.slice(0, 5).map((o) => (
                      <tr key={o.id} className="border-b border-border/40 last:border-0 hover:bg-primary/5 transition-colors">
                        ...
                        <td className="py-3 px-2 font-medium">{o.customerName || o.customerEmail}</td>
                        <td className="py-3 px-2 text-primary font-bold">₹{o.total.toLocaleString("en-IN")}</td>
                    ...
                    {orders.length === 0 && (
                      <tr>
                        <td colSpan={4} className="py-8 text-center text-muted-foreground">No orders received yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="rounded-2xl border border-border/60 bg-card p-6 transition-all duration-300 hover:border-primary/20 space-y-6">
              <div>
                ...
                <dl className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between border-b border-border/40 pb-2">
                    ...
                    <dd className="font-bold text-foreground">₹{avgProductPrice.toLocaleString("en-IN")}</dd>
                  </div>
                  <div className="flex justify-between border-b border-border/40 pb-2">
              ...
              <div className="pt-2 border-t border-border/40 space-y-2">
                <Button className="w-full btn-lift glow-pulse text-xs" onClick={() => setActiveTab("products")}>
                  Manage Products
                </Button>
                <Button variant="outline" className="w-full btn-lift text-xs border-primary/20" onClick={() => setActiveTab("orders")}>
                  Review Orders
                </Button>
              </div>
            </div>
      ...
      {/* ORDERS QUEUE TAB */}
      {activeTab === "orders" && (
        <section className={`mx-auto max-w-7xl px-4 pb-24 pt-6 sm:px-6 anim-reveal-up ${tabAnimate ? "visible" : ""}`}>
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-card transition-all duration-500 hover:border-primary/30">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                  ...
                  <tr key={o.id} className="border-b border-border/40 last:border-0 hover:bg-primary/5 transition-colors">
                    ...
                    <td className="p-4">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">{o.customerEmail}</div>
                      </div>
                    </td>
                    <td className="p-4 text-muted-foreground">
                      {o.items.reduce((s, i) => s + i.qty, 0)} items
                    </td>
                    <td className="p-4 font-bold text-primary">₹{o.total.toLocaleString("en-IN")}</td>
                    <td className="p-4">
                      <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${
                        o.status === "delivered" ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" :
                        o.status === "shipped" ? "bg-blue-500/10 border-blue-500/30 text-blue-400" :
                    ...
                    <td className="p-4 text-right">
                      <div className="inline-flex items-center gap-1.5">
                        <select
                          ...
                          className="h-8 rounded-lg border border-border/80 bg-background px-2 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-primary"
                        >
                        ...
                        <Button variant="ghost" size="icon" onClick={() => setOrderDetail(o)} className="hover:scale-105 hover:text-primary transition-all duration-200">
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleRemoveOrder(o.id)} className="hover:scale-110 hover:text-destructive transition-all duration-200">
                ...
                {orders.length === 0 && (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-muted-foreground">No orders placed on the system yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <section className={`mx-auto max-w-7xl px-4 pb-24 pt-6 sm:px-6 anim-reveal-up ${tabAnimate ? "visible" : ""}`}>
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-card transition-all duration-500 hover:border-primary/30">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {items.map((p) => (
                  <tr key={p.slug} className="border-b border-border/40 last:border-0 transition-all duration-300 hover:bg-primary/5">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        {p.image ? (
                          <img src={p.image} alt="" className="h-12 w-12 rounded-lg object-cover transition-transform duration-300 hover:scale-110" />
                        ) : (
                    ...
                    <td className="p-4 font-medium text-primary">₹{p.price.toLocaleString("en-IN")}</td>
                    <td className="p-4 text-right space-x-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(p)} className="hover:scale-110 hover:text-primary transition-all duration-200">
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => remove(p.slug)} className="hover:scale-110 hover:text-destructive transition-all duration-200">
                        <Trash2 className="h-4 w-4 text-destructive" />
                      </Button>
                    </td>
                  </tr>
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-muted-foreground">No products found. Add one above.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <section className={`mx-auto max-w-7xl px-4 pb-24 pt-6 sm:px-6 anim-reveal-up ${tabAnimate ? "visible" : ""}`}>
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-card transition-all duration-500 hover:border-primary/30">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {categories.map((c) => {
                  const pCount = items.filter(p => p.category.toLowerCase() === c.name.toLowerCase() || p.category.toLowerCase() === c.id.toLowerCase()).length;
                  return (
                    <tr key={c.id} className="border-b border-border/40 last:border-0 hover:bg-primary/5 transition-colors">
                      <td className="p-4 font-mono font-semibold">{c.id}</td>
                      <td className="p-4 font-medium text-foreground">{c.name}</td>
                      <td className="p-4 text-muted-foreground">{pCount} products</td>
                      <td className="p-4 text-right">
                        <Button variant="ghost" size="icon" onClick={() => handleRemoveCategory(c.id)} className="hover:scale-110 hover:text-destructive transition-all duration-200 cursor-pointer">
                          <Trash2 className="h-4 w-4 text-destructive" />
                        </Button>
                      </td>
                    </tr>
                ...
                {categories.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-muted-foreground">No categories setup. Add one above.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {customers.map((c) => {
                  ...
                  const activeCustOrders = custOrders.filter(o => o.status !== "cancelled");
                  ...
                  return (
                    <tr key={c.id} className="border-b border-border/40 last:border-0 hover:bg-primary/5 transition-colors">
                      ...
                      <td className="p-4 font-bold text-primary text-right">₹{totalSpent.toLocaleString("en-IN")}</td>
                      <td className="p-4 text-right">
                        <Button 
                          variant="ghost" 
                          size="icon" 
                ...
                {customers.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-muted-foreground">No customers in database.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <section className={`mx-auto max-w-7xl px-4 pb-24 pt-6 sm:px-6 anim-reveal-up ${tabAnimate ? "visible" : ""}`}>
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-card transition-all duration-500 hover:border-primary/30">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {coupons.map((c) => (
                  <tr key={c.code} className="border-b border-border/40 last:border-0 transition-all duration-300 hover:bg-primary/5">
                    <td className="p-4 font-bold text-primary tracking-wider uppercase">{c.code}</td>
                    <td className="p-4 font-medium text-foreground">
                      {c.type === "percentage" ? `${c.value}% Off` : `₹${c.value.toLocaleString("en-IN")} Off`}
                    </td>
                    <td className="p-4 text-muted-foreground">₹{c.minOrder.toLocaleString("en-IN")}</td>
                    <td className="p-4 text-muted-foreground truncate max-w-xs">{c.description || "—"}</td>
                    <td className="p-4">
                      {c.active ? (
                    ...
                    <td className="p-4 text-right space-x-1">
                      <Button variant="ghost" size="icon" onClick={() => openEditCoupon(c)} className="hover:scale-110 hover:text-primary transition-all duration-200">
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => removeCoupon(c.code)} className="hover:scale-110 hover:text-destructive transition-all duration-200">
                        <Trash2 className="h-4 w-4 text-destructive" />
                      </Button>
                    </td>
                  </tr>
                ...
                {coupons.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-muted-foreground">No active coupons found. Create one above.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {isEditingExistingProduct ? "Edit product" : "New product"}
            </DialogTitle>
          </DialogHeader>
          {editing && (
            <form
      ...
      <Dialog open={couponOpen} onOpenChange={setCouponOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {isEditingExistingCoupon ? "Edit coupon" : "New coupon"}
            </DialogTitle>
          </DialogHeader>
          {editingCoupon && (
            <form
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex justify-between items-center text-xl font-black">
                  ...
                  <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded-full border mr-4 ${
                    orderDetail.status === "delivered" ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" :
                    orderDetail.status === "shipped" ? "bg-blue-500/10 border-blue-500/30 text-blue-400" :
                    orderDetail.status === "cancelled" ? "bg-red-500/10 border-red-500/30 text-red-400" :
                    "bg-amber-500/10 border-amber-500/30 text-amber-400"
                  }`}>
                    {orderDetail.status}
                  </span>
              ...
              <div className="space-y-6 mt-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2 text-sm border-b border-border/40 pb-4">
                  <div>
                    <h4 className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-1">Customer Details</h4>
                  ...
                  <div>
                    <h4 className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-1">Shipping Address</h4>
                    <p className="text-muted-foreground">{orderDetail.shippingAddress?.address}</p>
                    <p className="text-muted-foreground">{orderDetail.shippingAddress?.city}, {orderDetail.shippingAddress?.postalCode}</p>
                  </div>
                </div>

                {/* Items details table */}
                <div>
                  <h4 className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-2">Order Items</h4>
                      ...
                      <tbody>
                        {orderDetail.items.map((item) => (
                          <tr key={item.slug} className="border-b border-border/40 last:border-0">
                            ...
                            <td className="p-3 text-right text-muted-foreground">₹{item.price.toLocaleString("en-IN")}</td>
                            <td className="p-3 text-right font-semibold">₹{(item.price * item.qty).toLocaleString("en-IN")}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="flex justify-end pt-2">
                  <div className="w-64 space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">₹{orderDetail.items.reduce((s, i) => s + (i.price * i.qty), 0).toLocaleString("en-IN")}</span>
                    </div>
                    <div className="flex justify-between border-t border-border/40 pt-2 text-lg">
                      <span className="font-bold">Total Paid:</span>
                      <span className="font-black text-primary">₹{orderDetail.total.toLocaleString("en-IN")}</span>
                    </div>
                  </div>
                </div>
              </div>
              <DialogFooter className="mt-6">
                <Button variant="pill" onClick={() => setOrderDetail(null)} className="btn-lift w-full sm:w-auto">
                  Close Invoice
                </Button>
              </DialogFooter>
            </>
