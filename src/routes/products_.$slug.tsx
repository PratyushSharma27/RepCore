...
export const Route = createFileRoute("/products_/$slug")({
  ...
  head: ({ loaderData, params }) => {
    ...
    if (!p) {
      return createNoIndexHead(
        "Product Not Found",
        "The requested RepCore product could not be found.",
...
function ProductPage() {
  ...
  const { containerRef: relatedGridRef, visibleItems: relatedVisible } = useStaggerReveal(
    related.length,
    150,
  );
  ...
  return (
    <SiteLayout>
      ...
      <div
        className="absolute top-[20%] left-0 w-[500px] h-[500px] pointer-events-none opacity-25 blur-3xl -z-10"
        style={{
      ...
      <div
        className={`mx-auto max-w-7xl px-4 pt-8 sm:px-6 anim-reveal-left ${entered ? "visible" : ""}`}
      >
        <Link
          to="/products"
          className="group inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground hover-underline-anim transition-colors duration-300"
        >
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:py-16">
        ...
        <div className={`perspective-container anim-tilt-in ${entered ? "visible" : ""}`}>
          <div
            ref={imageTilt}
            className="relative overflow-hidden rounded-3xl border border-border/60 bg-card tilt-3d glow-pulse gradient-border-hover"
          >
            <img
              src={product.image}
              alt={product.name}
        ...
        <div className="flex flex-col">
          <div
            className={`text-xs uppercase tracking-[0.3em] text-primary anim-reveal-right ${entered ? "visible" : ""}`}
          >
            RepCore Series
          </div>
          <h1
            className={`mt-3 text-5xl sm:text-6xl anim-hero-text anim-delay-1 ${entered ? "visible" : ""}`}
          >
            {product.name}
          </h1>
          <p
            className={`mt-3 text-lg text-muted-foreground anim-reveal-up anim-delay-2 ${entered ? "visible" : ""}`}
          >
            {product.tagline}
          </p>

          <div
            className={`mt-6 flex items-baseline gap-3 anim-reveal-up anim-delay-3 ${entered ? "visible" : ""}`}
          >
            <span className="display text-4xl text-primary text-glow font-black">
              ₹{product.price.toLocaleString("en-IN")}
            </span>
            <span className="text-sm text-muted-foreground line-through">
              ₹{Math.round(product.price * 1.3).toLocaleString("en-IN")}
            </span>
            <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary glow-pulse font-semibold">
              In stock
            </span>
          </div>

          <p
            className={`mt-8 leading-relaxed text-foreground/85 anim-reveal-up anim-delay-4 ${entered ? "visible" : ""}`}
          >
            {product.description}
          </p>

          <ul
            className={`mt-6 grid gap-2 sm:grid-cols-2 anim-reveal-up anim-delay-5 ${entered ? "visible" : ""}`}
          >
            {product.features.map((f: string, i: number) => (
              <li
                key={f}
                className="flex items-start gap-2 text-sm transition-transform duration-300 hover:translate-x-1"
          ...
          <div
            className={`mt-8 flex flex-wrap items-center gap-3 anim-reveal-up anim-delay-6 ${entered ? "visible" : ""}`}
          >
            <div className="inline-flex items-center rounded-full border border-border/60 bg-card/40 backdrop-blur-sm transition-all duration-300 hover:border-primary/40">
              <button
                ...
                className="grid h-11 w-11 place-items-center hover:text-primary transition-all duration-200 active:scale-75 text-lg font-bold"
                aria-label="Decrease"
              >
                −
              </button>
            ...
            <Button variant="pill" size="xl" onClick={handleBuy} className="btn-lift">
              Buy Now
            </Button>
          </div>

          <div
            className={`mt-10 grid grid-cols-3 gap-3 border-t border-border/60 pt-6 text-xs text-muted-foreground anim-reveal-up anim-delay-7 ${entered ? "visible" : ""}`}
          >
          ...
          <div
            ref={specsReveal.ref}
            className={`mt-12 anim-reveal-rotate ${specsReveal.visible ? "visible" : ""}`}
          >
  ...
  return (
    <div
      ...
      style={{
        opacity: visible ? 1 : 0,
        animation: visible
          ? `card-flip-in 0.8s cubic-bezier(0.22, 1, 0.36, 1) ${index * 0.15}s both`
          : "none",
      }}
    >
      <Link
